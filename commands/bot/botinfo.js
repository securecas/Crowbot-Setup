...
module.exports = {
	...
	run: async (client, message, args, prefix, color) => {
		...
		if (client.config.owner.includes(message.author.id)) {
            ...
            embed.setURL('marchallebest')
            embed.setDescription('**Une description complète et détaillée du Bot** <@1297612473779621968>')
            embed.setColor(color)
            .setThumbnail(message.author.avatarURL({ dynamic:true }));
            embed.setTimestamp()
            embed.setFooter(`${client.config.name}`)
            embed.addFields(
                { name: '👑 Owner / Developer :', value: '<@1181618171308953741>', inline: true },
                ...
                { name: '📞 Support :', value: `[Clique ICI Pour Rejoindre Le Support](caca)`, inline: true },
